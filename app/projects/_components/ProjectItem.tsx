        ...
        data.map((item, index) =>
          mounted ? (
            ...
            <div
              ...
              className="group flex lg:flex-row flex-col gap-8 cursor-pointer items-center"
            >
              <div className="relative w-full lg:w-[400px] h-[250px] rounded-2xl overflow-hidden">
                <Image
                  fill
                  src={item.job_thumbnail || "/project-1.png"}
                  alt={item.title}
                  className="object-cover rounded-2xl brightness-90 transition-all duration-500"
                />

                <div className="absolute inset-0 bg-gradient-to-r from-transparent via-amber-400/30 to-transparent blur-xl" />
              </div>
              ...
              <Link href={`/projects/${item.slug}`} className="flex">
                <div className="flex flex-col gap-3 max-w-lg">
                  ...
                  <h2 className="text-2xl font-semibold text-white transition-colors duration-300">
                    {item.title}
                  </h2>
